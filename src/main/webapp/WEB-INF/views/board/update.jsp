...
<body>
	<div class="container-fluid" style="padding: 0px">
		...
		<div class="container" id="wrapper" style="min-height: 100vh; margin-top:4vh; margin-bottom:4vh;">
			<div>
			<form action="update" method="post" onsubmit="return FormCheck()" class="d-flex flex-column justify-content-around" style="min-height: 90vh;">
				<div class="d-flex flex-column justify-content-around" style="height:20vh;">
				<div class="row">
				<div class="col-1">제목</div>
				<input class="form-control" type="text" name="title" maxlength="30" value="${board.title }" style="width:90%;">
				</div>
				<div class="row">
					<div class="col-1">종류</div>
				<select class="form-select" name="type" style="width:90%;">
  						<option value="자유" <c:if test="${board.type =='자유' }">selected="selected"</c:if> >자유게시판</option>
  						<option value="질문" <c:if test="${board.type =='질문' }">selected="selected"</c:if>>질문게시판</option>
  						<option value="정보" <c:if test="${board.type =='꿀팁' }">selected="selected"</c:if>>꿀팁게시판</option>
  						<option value="상품후기" <c:if test="${board.type =='상품후기' }">selected="selected"</c:if>>후기게시판</option>
					</select>
				...
				<div class="row">
					<div class="col">
					<textarea id = "description" name ="content" rows="5" cols="80">${board.content }</textarea>
				...
				<div class="row justify-content-around" style="margin-top:4vh;">
					<button class="col-5 btn btn-outline-primary" type="submit"><i class="bi bi-pencil-square"></i> 수정하기</button>
					<a class="col-5 btn btn-outline-warning" href="board"><i class="bi bi-card-list"></i> 목록으로</a>
				</div>
			</form>
			</div>
		</div>
