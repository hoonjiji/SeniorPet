	...
	<div class="container-fluid" style="padding: 0px">
		...
		<div class="container" style="min-height: 100vh; margin-top:4vh; margin-bottom:4vh;">
			<div class="d-flex my-2 justify-content-between align-items-center" style="min-height:15vh;
				...
				<div>
				<a class="btn btn-outline-secondary" href="board">전체</a>
				<a class="btn btn-outline-secondary" href="board?type=자유">자유</a>
				<a class="btn btn-outline-secondary" href="board?type=질문">질문</a>
				<a class="btn btn-outline-secondary" href="board?type=꿀팁">꿀팁</a>
				<a class="btn btn-outline-secondary" href="board?type=상품후기">상품후기</a>
				<a class="btn btn-outline-secondary" href="board?type=인기">인기글</a>
				<c:if test="${loginId != null }">
					<a class="btn btn-outline-secondary" href="board?id=${loginId }">내가 적은 글</a>
				</c:if>
				</div>
					...
					<tbody>
						<c:forEach var="board" items="${list }" varStatus="status">
							<tr style="cursor: pointer;"
								onClick="location.href='./read?boardnum=${board.boardnum}'">
								<td class="text-center">${status.count }</td>
								<td class="text-center">${board.type }</td>
								<td class="text-start">${board.title }</td>
								<td class="text-center">${board.id }</td>
								<td class="text-center">${board.inputdate }</td>
								<td class="text-center">${board.good }</td>
								<td class="text-center">${board.read }</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
			...
			<div>
			<form class="d-flex justify-content-center" action="board" id="pagingForm"method="get">
