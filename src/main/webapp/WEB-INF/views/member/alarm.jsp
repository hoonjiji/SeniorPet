		...
		<div class="d-flex" id="wrapper" style="min-height: 100vh;">
			...
			<table class="table table-hover text-center border">
				...
				<c:forEach var="alarm" items="${list }">
					<c:choose>
					<c:when test="${alarm.confirm =='N' }">
					<tr style="cursor: pointer;" onClick="location.href='read?boardnum=${alarm.boardnum}'">
						<td class="align-middle">${alarm.inputdate }</td>
						<td>${alarm.content }</td>
					</tr>
					</c:when>
					<c:when test="${alarm.confirm =='Y' }">
					<tr class="bg-secondary bg-opacity-25" style="cursor: pointer;" onClick="location.href='read?boardnum=${alarm.boardnum}'">
						<td class="align-middle">${alarm.inputdate }</td>
						<td>${alarm.content }</td>
					</tr>
					</c:when>
				...
				<c:if test="${list == null }">
					<tr>
						<td colspan ="2" class="fs-3 fw-bold">${Msg }</td>
					</tr>
				</c:if>
			</table>
 			</div>
